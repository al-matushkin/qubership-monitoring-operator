...
6. **Chain of events and root cause**
   ...
   **Findings (shop-checkout PoC, 2026-06-18 — external RCA via Aurora):**
   ...
   - **PoC implementation:** [`aurora-rca-stub.yaml`](../examples/keep-shop-checkout-poc/k8s/aurora-rca-stub.yaml), [`aurora-rca-rule-trigger-workflow.yaml`](../examples/keep-shop-checkout-poc/keep/aurora-rca-rule-trigger-workflow.yaml) — rule `created` only: trigger + inline poll to complete. **No Aurora workflows on topology** (2026-06-18).
...
7. **Notification templates**
   ...
   **Findings (shop-checkout PoC, 2026-06-11 — SMTP, one email per incident):**
   ...
   - **SMTP (rule only):** `shopchk-*` → `smtp-notification-workflow.yaml`, trigger **`created`**. Topology incidents have **no** SMTP workflow (2026-06-18). Use quoted string `if` for `incident_type` (bare `{{ incident.incident_type == 'rule' }}` renders empty in Keep 0.52.x).
