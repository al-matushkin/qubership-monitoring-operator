...
2. **Topology setup**
   ...
   **Findings (shop-checkout PoC, 2026-06-09 — topology processor behavior):**
   ...
   - **Graylog enrichment:** incident rollup (`log_summary`) runs on **rule** `incident:created` only — not on topology `updated` (2026-06-18: topology stripped of workflows).
...
6. **Chain of events and root cause**
   ...
   **Findings (shop-checkout PoC, 2026-06-18 — external RCA via Aurora):**

   - **Logical next step if Keep is adopted:** treat Keep as the **correlation + context hub** and plug in an **external RCA engine** (Aurora or similar) via workflows — trigger on incident create, poll until analysis completes, enrich `rca_summary` / `root_cause` back onto the Keep incident. Keep OSS does not replace that engine; it orchestrates and displays results.
   - **PoC implementation:** [`aurora-rca-stub.yaml`](../examples/keep-shop-checkout-poc/k8s/aurora-rca-stub.yaml) + [`smtp-notification-workflow.yaml`](../examples/keep-shop-checkout-poc/keep/smtp-notification-workflow.yaml) — rule `created` only: inline Aurora POST + poll, then email. **No Aurora workflows on topology** (2026-06-18; separate rule workflows removed 2026-06).
   - **Official Aurora docs:** [Arvo AI Aurora (Mintlify)](https://arvo-ai-aurora.mintlify.app/), [GitHub](https://github.com/arvo-ai/aurora). Real deploy is substantially heavier than the stub (LLM key, Vault, workers, UI).
   - **Keep “External incident” UI:** built-in link block uses enrichments `incident_id`, `incident_url`, `incident_provider` — appropriate slot for Aurora UI deep links in production. PoC sets custom `aurora_*` fields; `aurora_url` points at in-cluster JSON API (not browser-clickable).
   - **Workflow pitfalls (Keep 0.52.x):** do not use `keep.join()` inside HTTP JSON bodies; use `mock` + `enrich_incident` for merge-safe enrichment (HTTP `force: true` **replaces** entire enrichment dict and wiped `log_summary`); read enrichment flags in `if` via HTTP `GET /incidents/{id}`, not `incident.enrichments.*`; RCA is async — needs trigger + poll, not one-shot.
   - **Compared to alternatives evaluated for this checklist:** Aurora fits the **enrich-back** pattern best among OSS RCA tools for a Keep sidecar; Keep Incident Assistant is supplementary (LLM chat), not a substitute for a dedicated investigation platform.
   - **Still open for production:** deploy real Aurora; map `incident_url` / `incident_provider` for UI; add `rca_summary` / `root_cause` to incident sidebar env if desired; evaluate Aurora vs Enterprise Keep AI assistant cost/ops.
...
7. **Notification templates**
   ...
   **Findings (shop-checkout PoC, 2026-06-11 — SMTP, one email per incident):**
   ...
   - **SMTP (rule only):** `shopchk-*` → `smtp-notification-workflow.yaml`, trigger **`created`**. Integrated pipeline: wait for ≥1 linked alert, Graylog rollup, optional Aurora, one HTML email. Topology incidents have **no** SMTP workflow (2026-06-18). Use quoted string `if` for `incident_type` (bare `{{ incident.incident_type == 'rule' }}` renders empty in Keep 0.52.x).
   - **Template keys at `created`:** the integrated workflow builds email content in Python steps (`build-email-sections`) so templates do not reference `incident.log_summary` before enrichment runs. For simpler workflows, use incident-only fields at `created` or a `notification_sent` guard.
   ...
   - **Topology one-shot pattern (2026-06-08, superseded for shop-checkout):** separate topology SMTP workflow — removed; use rule integrated SMTP instead.
   ...
   - **Still open:** Jira/Webex/Slack providers not exercised in shop-checkout PoC (SMTP only); whether VMAlertmanager email templates remain primary for ops and Keep workflows only for incident-shaped notifications.
...
- **shop-checkout PoC** ([`docs/examples/keep-shop-checkout-poc/`](../examples/keep-shop-checkout-poc/)): three-service app (`storefront` → `checkout-demo` → `payments-api`), manual Keep topology YAML, CSV mapping (`runbook_url`, `owner`, `escalation_tier` on top-level alert fields — see item 4), `ALERT_SIDEBAR_FIELDS` in `values-keep-kind.yaml` for UI sidebar visibility (extraction not needed for shop alerts — see item 5), Graylog log enrichment workflow, topology/rule correlation for symptom grouping (RCA is human/AI-assisted in Keep OSS — see item 6; optional **Aurora RCA stub + workflows** demonstrate external RCA enrich-back), and Kind logging stack (OpenSearch + logging-operator). Reproduce with `keep/apply-keep-config.sh` and `validate.sh`; Aurora path: `k8s/aurora-rca-stub.yaml` + `keep/test-aurora-rca.sh`.
